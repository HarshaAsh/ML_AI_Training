...
def greedy_fractional(items, max_weight, key_function):
    ...
    for i in range(len(sorted_items)):
        ...
        if(total_weight+cur_item.get_weight() <= max_weight):# if we can fit the entire item into knapsack, do it, update its weight and value
            result.append({cur_item,1}) # code: add item to knapsack
            total_value += value(cur_item) # code: update knapsack weight and value
            total_weight += cur_item.get_weight()
        elif(total_weight < max_weight):  # If cannot fit full item, fill knapsack to capacity
            fraction = (max_weight-total_weight)/(cur_item.get_weight())
            if(fraction>0 and fraction < 1):
                result.append({cur_item,fraction}) # code: add item to knapsack
                total_value += value(cur_item)*fraction # code: update knapsack weight and value
                total_weight += cur_item.get_weight()*fraction
...
def greedy_heuristic(items, max_weight, key_function, max_players):
    ...
    def add_item_to_list(result, cur_item, total_value, total_weight):
        result.append(cur_item) # code: add item to knapsack
        total_value += value(cur_item) # code: update knapsack weight and value
        total_weight += cur_item.get_weight()
        return result, cur_item, total_value, total_weight
    ...
    for i in range(len(sorted_items)):
        ...
        if((total_weight+cur_item.get_weight() <= max_weight)):
            if(cur_item.position == 1 and count_goal < max_goal):
                count_goal+=1
                result, cur_item, total_value, total_weight = add_item_to_list(result, cur_item, total_value, total_weight)
            elif(cur_item.position == 2 and count_def < max_def):
                count_def+=1
                result, cur_item, total_value, total_weight = add_item_to_list(result, cur_item, total_value, total_weight)
            elif(cur_item.position == 3 and count_mid < max_mid):
                count_mid +=1
                result, cur_item, total_value, total_weight = add_item_to_list(result, cur_item, total_value, total_weight)
            elif(cur_item.position == 4 and count_fw < max_fw):
                count_fw+1
                result, cur_item, total_value, total_weight = add_item_to_list(result, cur_item, total_value, total_weight)
